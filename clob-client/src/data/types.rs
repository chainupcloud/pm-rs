//! Response types for the chainup `data-service`. Field shapes match the Go handler structs
//! at `pm-cup2026/services/data-service/internal/handlers/` 1:1 — including chainup-specific
...
pub struct Position {
    ...
    pub mergeable: bool,
    pub title: String,
    /// chainup i18n extension — JSON-encoded `{"zh_CN":"...","en_US":"..."}`.
    #[serde(default)]
    pub question_translation: String,
    /// chainup i18n extension.
    #[serde(default)]
...
pub struct Trade {
    ...
    /// chainup extension — fee in USDW. Non-omitempty.
    pub fee: f64,
    #[serde(default, skip_serializing_if = "String::is_empty")]
    pub event_type: String,
    #[serde(default, skip_serializing_if = "String::is_empty")]
...
/// Wrapped envelope returned by `GET /v1/leaderboard`. The chainup deviation from polymarket
...
/// USDW unwrap-request row. Chainup-only — no polymarket equivalent.
#[derive(Debug, Clone, Serialize, Deserialize, PartialEq)]
#[serde(rename_all = "camelCase")]
