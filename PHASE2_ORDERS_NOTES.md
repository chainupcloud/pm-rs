...
2. **`OrderBuilder::market` does not walk the book client-side.** Polymarket V1's `MarketBuilder::calculate_price` walks asks/bids to find the cutoff price; the server runs the actual book-walk, so the client signs at an anchor price. Callers that want the V1-style depth check should call `Client::book(...)` themselves.
